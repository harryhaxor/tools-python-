...
def SixFiveZero(site):
    ...
    try:
        ...
        for path in Pathlist:
            ...
            if "/wp-" in response.text or '/WP-' in response.text:
                print(f"\t{Fore.GREEN}[✓]{Style.RESET_ALL} {site} => {Fore.GREEN}WORDPRESS{Style.RESET_ALL}")
                open(f"WordPress-{date}.txt", "a").write(url)
            else:
                print(f"{Fore.RED}[✗]{Style.RESET_ALL} {site} => {Fore.RED}OTHERS{Style.RESET_ALL}")
    except:
        pass
def main():
...
\___ \| \ \/ /___ \ / // _ \ '__/ _ \ 
 ___) | |>  < ___) / /|  __/ | | (_) |
|____/|_/_/\_\____/____\___|_|  \___/ {Fore.RED}CMS CHECKER{Style.RESET_ALL}
